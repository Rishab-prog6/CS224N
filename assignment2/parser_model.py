...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.empty(self.hidden_size, self.n_features * self.embed_size))#Parameter第一个参数是数据，第二个默认requires_grad=True
        #torch.empty中所有数字的参数都是传到第一个参数里，其余的都是关键词参数，这个array的shape是（hidden_size, n_features * embed_size）,而内容是未初始化的。
    ...
    def embedding_lookup(self, w):
        ...
        flat_w = w.view(-1) # (batch_size * n_features)
        x = torch.index_select(self.embeddings, 0, flat_w)
        ###     2) Reshape the tensor using `view` function if necessary
        x = x.view(w.shape[0], self.n_features * self.embed_size) # (batch_size, n_features * embed_size)
        ###
...
#        x = None
    ...
    def forward(self, w):
        ...
        #matmul is tensor product 
        h = torch.matmul(x, self.embed_to_hidden_weight.t()) + self.embed_to_hidden_bias #wx + b
        h_relu = F.relu(h)
